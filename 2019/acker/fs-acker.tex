...
In distributed stream processing, it is hard to determine a correspondence between input and output elements due to complex transformations, filterings, producing multiple items from a single, etc. On the other hand, mechanisms for taking consistent state snapshots and for transactional stream processing require notifications when the specific set of input elements have been already processed. To produce these notifications, a system needs to track dependencies between input items and their descendants. In this work, we propose a method for monitoring dependencies between streaming elements that can be applied for both cyclic (iterative) and acyclic dataflows. Instead of a punctuation-based approach where special elements are injected directly into a data flow and push input descendants through a system, we enrich all data items with logical timestamps that identify input items they derive from. We build a scalable method to watch for the computational progress and to notify when data flow does not contain elements with specific timestamps, i.e. when all descendants of concrete input elements have been completely processed. We demonstrate that our technique provides low notification latency while having almost no throughput overhead on regular processing, and can outperform methods employed by state-of-the-art stream processing systems.
