...
Change detection algorithms are widely used in dataflows that monitor suspicious activity in credit card transactions, stock market ticks, or computer network traffic. Another application is to track unexpected fluctuations in data used for training machine learning models. To make these dataflows scalable, it is natural to run them on distributed stream processing systems such as Flink, Storm, Spark Streaming, etc. However, most change detection algorithms aim to a case when all data stream items are handled by a single worker. In this work, we complement an existing efficient change detection algorithm to be suitable for the case when streaming data is partitioned among workers. Thus, it is possible to use our algorithm within a distributed stream processing engine for the price of a larger false detection rate. We conduct a series of experiments with several data partition methods and demonstrate an emerged trade-off between scalability and change detection accuracy.
