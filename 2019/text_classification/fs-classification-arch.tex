...
\begin{itemize}
    \item There can be a race between elements before the update of {\em IDF} due to multiple asynchronous network channels as it is shown in~\ref{physical_graph}. Unlike batch processing systems, where sorting data before an operation is natural, in stream processing systems such behavior is not commonly used due to strong low latency requirements. It is unclear how such reorderings during the physical execution influence classification results.
