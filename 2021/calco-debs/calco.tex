...
\title {Contracts-based approach to define and optimize computational graphs in distributed stream processing}
...
\begin{abstract}
    ...
    Distributed stream processing is a common way to solve real-time data analysis and management objectives.
    However, computational graph optimization is the thing that state-of-the-art stream processing engines (SPEs) still do not do in the ETL (extract, transform, load) use cases.
    One of the reasons is the lack of information about the defined graph, and SPE is not able to permute operations to achieve better performance to avoid graph semantics corruption.
    So graph optimization is generally a programmer concern.
    But computational graphs in real-world production applications are usually rather big to optimize them manually.
    ...
    In this work, we introduce a new approach to define computational graphs.
    It provides significantly more information for SPE, hence it allows to generate all graphs with the needed semantics to be able to choose the most optimal one.
    We demonstrate convenience for programmers to use this approach and present a prototype implementation.
