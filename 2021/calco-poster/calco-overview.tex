...
Semantics allows us to reduce such set and to accelerate generation.

Execution graphs generation algorithm implementation is rather effective despite brute force approach, because graphs that does not satisfy contracts can be ejected and exponential complexity depends only on the size of the semantics set that is always small enough.
