...
Such specifications we call contracts.
Input contract specifies requirements that input data should satisfy, output contract defines how node changes the input data.

A data stream can be described using two sorts of information: attributes that every data element has and properties of such attributes.
...
We call the nodes that produce such side effects as a {\em graph semantics}.
...
Graphs with satisfied contracts that include all semantics nodes form the desired set of equivalent graphs. Execution graphs generation algorithm implementation is rather effective despite brute force approach, because graphs that does not satisfy contracts can be ejected and exponential complexity depends only on the size of the semantics set that is always small enough.
