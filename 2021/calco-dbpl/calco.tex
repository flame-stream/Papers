...
\begin{abstract}
    Distributed dataflows can be specified in two ways: by defining an execution graph or by specifying a desired result (we will consider an SQL notation as an example).
    Execution graph consists of the arbitrary operations, so it cannot be automatically transformed in performance purposes.
    % by a distributed data processing system
    ...
    Such specifications allow us to automatically generate a set of the equivalent execution graphs to choose optimal one.
