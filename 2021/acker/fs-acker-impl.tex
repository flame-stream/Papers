...
In the previous section, we introduced a general schema of the \tracker\ framework. In this section, we deepen into its implementation details. We describe the in-flight elements monitoring mechanism and explore its properties. We detail the spawn technique for the substream end notifications. After that, we focus on the single node implementation and local agent optimization. Distributed implementation of the tracking agent concludes this part.

\subsection{Monitoring of in-flight elements}
...
The tracking agent aggregates this information into the table illustrated in Table~\ref{tracker-table-simple}. In this table, for each predicate, there is information if a {\em graph segment} contains in-flight elements satisfying the predicate. Segments are subgraphs that incapsulate cycles. Substream end notifications can be sent for a subgraph if all upstream subgraphs do not contain the corresponding elements. The substream ends for the whole graph if all segments do not contain elements satisfying the prodicate~\footnote{Data source should also guarantee that it will not spawn any elements from this substream}. Therefore, the notifications for predicates $h(x)$ and $z(x)$ should be generated.

\begin{table}[htbp]
\caption{\tracker\ table: an example}
...
% \begin{table}
% \caption{\tracker\ table}
%   \label{tracker-table-xor}
%   \centering
%   \begin{tabular}{|c|c|c|c|c|} 
%     \hline
%     Notified & Predicate & Segment & Local XOR & XOR  \\ \hline \hline
...
% \begin{table}
% \caption{\tracker\ table for consistent notifications order}
%   \label{tracker-table-oder}
%   \centering
%   \begin{tabular}{|c|c|>{\bfseries}c|c|c|c|} 
%     \hline
%     Notified & Predicate & t(x) & Segment & Local XOR & XOR  \\ \hline \hline
%     \multirow{2}{*}{\checkmark} & \multirow{2}{*}{h(x)} & \multirow{2}{*}{1} & A & 000 & \multirow{2}{*}{000} \\ \cline{4-5}
%     & & & B & 000 & \\ \hline
%     \multirow{2}{*}{} & \multirow{2}{*}{q(x)} & \multirow{2}{*}{2} & A & 000 & \multirow{2}{*}{110} \\ \cline{4-5}
%     & & & B & 110 & \\ \hline
%     \multirow{2}{*}{blocked} & \multirow{2}{*}{z(x)} & \multirow{2}{*}{3} & A & 000 & \multirow{2}{*}{000} \\ \cline{4-5}
%     & & & B & 000 & \\ \hline
%   \end{tabular}
% \end{table}
