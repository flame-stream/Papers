...
% {\noindent \bf Overall performance scenarios.} 
...
% The second scenario is hot keys detection. The distribution of keys of data elements that entering SPE can be skewed, and some (hot) keys can occur very often. In this case, load distribution between computational nodes may be imbalanced, and the throughput of an SPE may decrease~\cite{liao2019efficient}. If SPE can detect hot keys, it can forbid entering elements with such keys or rebalance load distribution. In this scenario, we evaluate \tracker\ latency and accuracy of hot keys detection. As a baseline approach, we use a naive method that counts keys occurrences independently on each computational node.
