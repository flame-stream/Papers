...
We assume that within the same process $p$ all events are totally ordered by a local causal order relation $<_p$: $e^{0}_p,e^{1}_p,...,e^{i}_p,...$. This assumption is naturally satisfied in a single-thread environment. To effectively use multiple threads, we can settle multiple processes into a single node~\footnote{Actor systems are a good practical example of such settlement}.
