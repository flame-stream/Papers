...
If this condition is satisfied, then $\langle eos_{firm}, pred(m)\rangle$ = $\langle eos_{soft}, pred(m)\rangle$ for the punctuations. The punctuations framework provides consistent notification order by design because punctuations are naturally ordered with ordinary data elements within the processes.
...
Such elements broadcasting may affect the throughput of an SPE~\cite{Li:2008:OPN:1453856.1453890}. As we demonstrate further, the punctuation technique adds significant performance overhead on regular processing for small substreams (frequent punctuations injection).
