...
It is useful to view an infinite stream as a mixture if finite substreams.
...
Formal framework for stream processing in general.

\subsection{Punctuations model}

Formal framework for punctuations.

\section{Tracker overview}

\subsection{Dividing into substreams via ordering}

Show how to divide stream into substreams using order on streaming elements.

\subsection{Agent for tracking substreams}

\tracker basics in terms of formal framework.

\subsection{\tracker\ properties}

Show that our framework has almost the same properies as punctuations, but do not require frequent network broadcasting.

\section{Tracker mplementation}

\subsection{Global time and metadata}
