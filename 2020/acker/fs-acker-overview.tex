...
\begin{enumerate}
    ...
    \item {\bf Low network overhead.} \tracker\ does not require frequent broadcasting of the elements to all computational nodes. As we show in the next section, the amount of service network needed for \tracker\ is $O(N+C)$, where $N$ is the length of a dataflow, and $C$ is the number of computational nodes. 
\end{enumerate}

At the same time, \tracker\ is able to provide both general and strict guarantees along with the consistent order of notifications. A limitation of the \tracker\ framework is a potentially more complex implementation than punctuations. The details of the straightforward implementation of the \tracker\ framework are discussed in the next section.
