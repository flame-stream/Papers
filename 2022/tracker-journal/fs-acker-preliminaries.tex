...
\end{lemma}
\begin{proof}
Let us suppose that all punctuations for the predicate $pred$ had been arrived to a process $p$ and all conditions to generate soft bound event $e_{soft} = \langle eoss_{soft}, p, Pred\rangle$ are satisfied:

\begin{equation}
\forall q \in I_p, \exists \mathcal{P}^{pred}_{qp} \in B_p, \forall M\in B : \neg pred(M) \vee dst(M) \ne p
\end{equation}

However, having a cycle in a graph, we cannot guarantee that messages satisfying the predicate $pred$ will not return through the cycle even if all network channels are FIFO. Hence, there can be an event $e^{*} = \langle recv, p, M^{*}\rangle$ such that $e^{*} > e_{soft}$ and $pred(M^{*})$ that contradicts the definition of the soft bound event.
...
Even the soft bound event requires receiving punctuations from all incoming network channels. If a distributed stream processing engine balances the work evenly among the processes, all processes are interconnected. Therefore, each process should broadcast punctuations to all other processes. Therefore, if the number of existing substreams is $K$ then the network traffic complexity for the punctuations framework is $O(K|\Pi|^2)$ because each process should broadcast punctuations for each substream. 
