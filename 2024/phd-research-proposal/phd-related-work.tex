...
\paragraph{Handling out-of-order events}\mbox{} \\

% In stream processing, there are two concepts of time: event time and processing time. Event time refers to the time at which an event actually occurred outside an SPE. This timestamp is usually embedded within the event data itself. Event time is essential for applications that require precise temporal analysis, such as user behavior analysis, billing systems, or anomaly detection. It allows for the construction of a true timeline of events, even if those events are ingested into the system out of order. On the other hand, processing time is the time at which an event is observed and processed by the stream processing system. This is typically the system clock time at the moment the event is processed. It can vary due to factors like network latency, system load, and event source variability. Processing time is often simpler to manage and can be sufficient for use cases where exact temporal accuracy is not critical. However, relying solely on processing time can lead to inaccuracies if events are delayed or arrive out of order.
