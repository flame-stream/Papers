...
Facing this fact, let us try to dive into the internals of stream processing. Let $\tau$ be an exact global discrete time. Let $a_\tau$ be the element, which enters at the time $\tau$, and $b_\tau$ is the element, which leaves at the time $\tau$. Let $W_\tau$ be the set of elements, which are currently in the system, i.e.,

$W_{\tau+1}=\begin{sqcases}
W_{\tau}\cup{a_\tau}, & \text{or}\\
W_{\tau}\setminus{b_{\tau+1}}, & \text{or}\\
W_{\tau}\setminus{x}\cup{\{y_i\}}, x\in{W_\tau},{\{y_i\}}\in{W_{\tau+1}} & \text{}.
\end{sqcases}$

Basically, the last expression illustrates the case when an element transforms into other elements due to, e.g., map operation. But as it was mentioned above, we assume that the states of operations are elements in the system as well, so such transformation can also describe combining an element with the business logic state. In this setting, we can define operations state $S_\tau$ as $W$ if there are no input elements since time $\tau$, i.e. $\forall t>\tau: a_t=\emptyset$. Let us denote it as $S_\tau=W^{\infty}_\tau$. We can imagine stream processing system as a pool, where some elements are poured in and others are poured out. The business logic states are the elements, which are kept in the pool if we stop pouring in and wait infinite time. Regarding this model, we can use $W_\tau$ as a system state, because it contains both operations state and all elements, which are currently in the system, i.e., it contains a complete information about the elements, which have been already entered.

The key problem within this model is that using only defined terms, we cannot observe a moment, when an input element starts to affect output elements only through the state. In other words, it is the moment, when all information about this input element is started to be contained only in business-logic state. Further in this paper, we will call this moment {\em nullification}. The lack of nullification is illustrated in Figure~\ref{nullification}. In order to define this event, let us introduce the notion of {\em dependency}. It can be said that an element depends on other one, if the element is a result of other element transformation. Let us denote this as a transitive binary relation $D$. Let $Cl(D)$ be a transitive closure of $D$. We say that input element $a_\tau$ is nullified at the time $\theta_{a_{\tau}}$ if $\theta_{a_{\tau}}=inf(\hat{\tau}>\tau|W_{\hat{\tau}}\setminus{W^{\infty}_{\hat{\tau}}}\cap{Cl(D)(a_\tau)=\emptyset})$, i.e., if at the time $\theta_{a_{\tau}}$ there are no elements in a system, which depend on $a_\tau$, except business-logic state.
