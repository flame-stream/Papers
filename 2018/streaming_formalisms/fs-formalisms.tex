...
The main purpose of user state is to accumulate the information about input items. In most systems, user state is stored on disk or in external system, e.g., key-value storage. It is commonly assumed that $\forall{\tau:} ||{W_\tau}\setminus{S_\tau}||<m<<||S_\tau||$, where $m$ is the memory size of all computational units. Hence, for each input element $a_\tau$, there must be a nullification time $\theta_{a_\tau}$, thereafter all elements, which depend on $a_\tau$, are in user state. Since the nullification time, the input element can affect output elements only through the state. The concept of nullification is shown in Figure~\ref{nullification}.   
