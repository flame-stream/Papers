...
Data producers are external to our systems, {\em front} is a component that acts as merdiator between them and a computational pipeline.

At the end of pipeline, there is a {\em barrier} that prepares results to be consumed by end-user. The main role of a barrier is to filter out possible duplicates or invalid results that can appear due to optimistic mechanisms. Another function is to carry out an output protocol between system and data consumers, which is essensial part of providing exactly-once semantics.
