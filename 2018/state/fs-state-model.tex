...
At the first glance, total order maintenance is over-restrictive. It can be achieved, e.g., using buffering before each order-sensitive operation until punctuation or low watermark arrives ~\cite{Li:2008:OPN:1453856.1453890}. However, frequent buffering can dramatically increase latency. 
...
\begin{itemize}
    ...
    \item Map is stateless operation and grouping maintains sorted on meta-information lists of data items. Only system itself can manage this state, so drifting state model is stateless from a user point of view.
    \item In spite of reduced set of operations, this model is sufficient to express any stateful transformation.
