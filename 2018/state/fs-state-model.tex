...
\begin{itemize}
    ...
    \item There are only two operations: map and windowed grouping. The set of dependencies $D$ is generated only by these operations.
    \item User-defined code can only be present in map operation, while for grouping, a user should only define hash balancing function. Drifting state model is stateless from a user point of view.
    \item Grouping operation only maintains a system state. The state is represented by sorted on meta-information lists of data items.
