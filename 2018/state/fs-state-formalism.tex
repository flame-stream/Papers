...
We suppose that {\em snapshots} of operation states taken by real stream processing systems become ordinary output elements. Thus, we allow using output elements in system transformations. The rational for this assumption is the following:

\begin{itemize}
    \item Snapshots shoe be considered as an output as they are usually  stored in external systems, (such as in HDFS, Kafka, relational databases)   and are read back during recovery. 
