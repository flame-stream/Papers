...
\begin{enumerate}
    ...
    \item \textbf{Transform}\\ $A_{\tau + 1} = A_{\tau}$,\\ $B_{\tau+1}=B_{\tau}$, \\ $W^{*}_{\tau+1}=(W^{*}_\tau \setminus X_\tau) \cup Y_\tau, (X_\tau,Y_\tau) \in D$, \\where\\$X_\tau \thicksim \mathcal{F}(X\mid X \subseteq W^{*}_\tau \cup B_\tau , \exists Y : (X,Y) \in D)$, \\ distribution $\mathcal{F}$ depends on a system architecture. \label{random_formula}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item[(ii)] $((x,y),s)\in D$ through non-commutative operation
...
b_{\tau_2} \in Cl_D(b_{\tau_1}), b_{\tau_2} \notin Cl^{-1}_D(b_{\tau_1})    
\end{equation}
