...
is a tuple of $(\Gamma,D,W,P,F)$, where $\Gamma$ is all possible data flow elements, $D$ is dependency relation between elements, $W$ is a set of elements in a system, $P$ is a snapshot, and $F$ is recovery function. 
\end{definition}
...
$\widehat{W}_\tau\subseteq{\Gamma}$ at the time $\tau$ is the set of elements, which are currently in the system:

$\widehat{W}_0=\emptyset$:

$\widehat{W}_{\tau+1}=\begin{sqcases}
\widehat{W}_{\tau}\cup{a_\tau}, & \text{or}\\
...
t are time moments $\tau$ such that there is an output element at these moments, $t=\{\tau:\exists{b_\tau}\}$.
\end{definition}

\begin{definition}{System provides for exactly once}
if each output element $b_{t+1}$ is consistent with previous output, input and dependency relation, i.e., $p(b_{t+1}|B_t, A_{t+1}, D)>0$.
\end{definition}

\begin{definition}{System provides for at most once}
if $\exists{A^{0}_{t+1}\subseteq{A_{t+1}}}$ such that $\forall{b_{t+1}}, p(b_{t+1}|B_t, A^{0}_{t+1}, D)>0$.
\end{definition}

\begin{definition}{System provides for at least once}
if $\exists{A^{*}_{t+1}\subseteq{2^{A_{t+1}}}}$ such that $\forall{b_{t+1}}, p(b_{t+1}|B_t, A^{*}_{t+1}, D)>0$.
\end{definition}

Further we will consider mostly exactly once. We can proceed from the probability of output element to the probability of the working set through the law of total probability:

$p(b_{t+1}|B_t, A_{t+1}, D)=\\
\sum\limits_{W_{t+1}}p(W_{t+1}|B_t,A_{t+1},D)p(b_{t+1}|B_t,A_{t+1},W_{t+1},D)=\\
\sum\limits_{W_{t+1}}p(W_{t+1}|B_t,A_{t+1},D)p(b_{t+1}|W_{t+1},D)
$.

\begin{definition}{Snapshot P is consistent}
if $\forall{t} \exists{A^{p}_t\subseteq{A_t}} : p(W_{t+1}=F(A^{p}_t,P_t)|B_t,A_{t+1},D)>0$.
\end{definition}

\begin{theorem}
If $\forall{t} \exists{A^{p}_t\subset{A_t}} : F(A^{p}_t,P_t)=\widehat{W_t}$, then snapshot $P$ is consistent.
\end{theorem}
...
$P^{s}$ is a snapshot such that $\forall{\tau} P^{s}_\tau = S_{\tau_s},\tau_s \leq \tau$.
\end{definition}

\begin{theorem}
The necessary conditions of consistent state snapshot are:\\
1. $\forall{y}\in{P^{s}_t}, \forall{a\in{Cl^{-1}(D)(y)}} : \exists{\theta_a}$\\
2. $\forall{a}\notin{Cl^{-1}(D)(P^{s}_t)} \wedge \nexists{\theta_a}:a\in{A^{p}_t}$
\end{theorem}

\begin{corollary}
System that uses state snapshot provides for exactly once only if $\forall{b_t} : P^{s}_t=S_t$.
\end{corollary}

\begin{definition}{System is deterministic}
if\\ 
$\forall{b_{t+1}} : p(b_{t+1}|B_t, A_{t+1}, D)=1$.
\end{definition}
