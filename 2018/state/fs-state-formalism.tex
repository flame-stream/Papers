...
Unfortunately, the reference stream processing system is just an abstract concept, that is not possible to implement due to probable failures of network and computational units. Failure can be considered as a loss of the working set or its part. There is a need to extend the reference model by recovering mechanism that allows a system to transparently pass through the failures.
