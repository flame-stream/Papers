...
As it was mentioned above, end-user cannot observe internal system state. We define streaming consistency guarantees only in terms of output elements. The ideal system is used to determine which of them are correct.
...
Exactly once states that observed results cannot be distinguished from the results of the ideal system. At most once and at least once guarantees are the relaxations of exactly once. The results within these guarantees can be obtained in the ideal system, but with the assumption, that input is not completely correct. At most once can be achieved in the ideal system if some input elements are missed. At least once can be reproduced if the input contains duplicates. 

We can proceed from the probability of output element to the probability of the working set through the law of total probability:
