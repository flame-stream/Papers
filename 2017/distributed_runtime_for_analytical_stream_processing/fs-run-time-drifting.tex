...
Most of the computational pipelines requires state carring between consecutive data items. For example, moving average calculation requires partial sums to be stored. The state management in distributed systems is a complicated topic that requires delicate treatment~\ref{some-state-handling}.

Some systems require that all state management is put in busines-logic: making state persistent, replication, ensuring state consistency etc. Others shifts some dificulties to the system's concern, e.g. Flink provides a state API that guarantees consistency in case of failures \ref{async-snapshots}. While it eases the heavy desteny of users, the core problem remains, there is a need to think about state.
