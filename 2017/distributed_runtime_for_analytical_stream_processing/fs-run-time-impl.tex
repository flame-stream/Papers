...
For any data item $D$ let  $ \cal {G} (D) $ be its global time. 
  If data item $D$  has global time $ \cal {G} (D)  > \cal {G} (F)$  for  each in-flight element $F,$ 
  then all tombstones  for that item had already arrived at the barrier.
\end{minimal-time-claim}

\begin{proof}
  Let    $D_{tomb}$ be an  tombstone for {\it D}. 
  According to the definition of the tombstone item, $\cal{G} (D_{tomb}) = \cal {G} (D),$   hence $D_{tomb}$ is not in-flight.
  
  New tombstones for $D$ cannot be generated because items with global time greater than $\cal{G} (D)$ cannot trigger repair that affects  $D.$
  This implies that if the stream does not contain items $D\prime$ such that $ \cal{G} (D\prime \le \cal{G} (D) ,$ then all tombstones for $D$ had already arrived at the barrier. 
\end{proof}
...
Therefore, the barrier can release elements with global time $\cal{G} (D)$ 
once it received notification with time greater than $\cal{G} (D).$
