...
\item [Grouping] constructs a single item containing a set of consequtive items that have the same value of load balancing function. The maximum number of items that can be grouped is specified as a parameter and is called window size. 
\end {description}
...
The grouping operation has two important properties: the output tuple is identified by its last element, the results among items with different values of a load balancing function are independent.
