...
\label {fs-model}
In this section we describe the structure of the proposed system. We will focus on operation set definition and computational layout. The guarantee mechanisms will be discussed in separate section~\ref{fs-collision}.
...
The concept of ordering is shown in Figure~\ref{ordering}. Data item with payload $1'$ is the image of the item with payload $1$, according to operation $F$. The same is satisfied for items with payloads $2'$ and $2$. After union operation, the order between $1$ and $2$ is preserved. Furthermore, $1'$ is located right after $1$, and $2'$ is located right after $2$.  
