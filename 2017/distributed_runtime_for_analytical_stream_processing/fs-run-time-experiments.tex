...
Thus, Flink can be more appropriate if there is a need to optimize computational resources, but the demands on latency are not very strict, or determinism is not required. \FlameStream\ is more relevant for cases when low latency and determinism are strict requirements, but an increase of computing power is not a problem.  
