...
Much of the hierarchy --- A2, A3, E2-seq, and compensation-based commit
spheres --- presumes that effects have compensations. In practice agent
tools rarely ship with them; they arrive as bare API calls. The
achievability of the compensation-based levels therefore hinges on whether
...
purchase admits none)? This bounds where E2-seq is reachable at all. Second,
...
practical gate on whether the compensation-based levels are inhabited in
real deployments.
...
instance, assumes a tool has no side effect when it cannot find a
compensator. This suggests enriching $P$ with an epistemic status for each
...
replaying one wrongly believed idempotent. Characterizing isolation under
